...
# =====================================================================
# 2. TABLE ACTIVATION MODULE FROM A READ CSV FILE
# =====================================================================
class CSVTabularActivation(nn.Module):
    def __init__(self, csv_path="SigmaExp.csv"):
        super(CSVTabularActivation, self).__init__()
        ...
        # We extract the grid parameters from the file itself
        self.x_min = float(x_values[0])
        self.x_max = float(x_values[-1])
        self.num_points = len(x_values)
        self.step = (self.x_max - self.x_min) / (self.num_points - 1)
        
        # We register Y as a PyTorch buffer so that it is automatically offloaded to the CUDA GPU
        self.register_buffer('y_grid', torch.from_numpy(y_values.copy()).float())

    def forward(self, x):
        # Safe clipping to prevent indices from going beyond the array bounds
        x_clipped = torch.clamp(x, self.x_min, self.x_max - 1e-5)
        
        # Calculate the fractional index of a point's position in the table
        float_index = (x_clipped - self.x_min) / self.step
        
        # Find the left and right nodes for interpolation
        idx_low = torch.floor(float_index).long()
        idx_high = idx_low + 1
        
        # Offset coefficient between nodes
        weight = float_index - idx_low.float()
        
        # Interpolate the value of the function
        y_low = self.y_grid[idx_low]
        y_high = self.y_grid[idx_high]
        
        return y_low + weight * (y_high - y_low)

# =====================================================================
# 3. TABLE-BASED ACTIVATION NETWORK ARCHITECTURE
# =====================================================================
class FinalPerfectPerceptron(nn.Module):
    def __init__(self, input_dim=784, hidden_dim=HIDEM, output_dim=10):
        ...
        # CHANGE: We're linking our activation from the generated CSV file
        self.custom_activation = CSVTabularActivation("SigmaExp.csv")
    ...
    def forward(self, x):
        ...
        # Calling an array function instead of a standard one
...
# =====================================================================
# 4. TRAINING AND CLASSICAL METRICS (OPTIMIZER ERROR FIXED)
# =====================================================================
...
print("Getting Started with CSV Data...")
